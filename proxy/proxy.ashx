...
public class proxy : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        bool parseCapabilitiesXml = false;
        string searchUrl = "http://wmts.geo.admin.ch/1.0.0/WMTSCapabilities.xml";
        if (context.Request.RawUrl.Contains(searchUrl))
        {
            parseCapabilitiesXml = true;
        }              
            ...
            using (Stream byteStream = serverResponse.GetResponseStream())
            {
                    ...
                    serverResponse.ContentType.Contains("xml"))
                {
                    using (StreamReader sr = new StreamReader(byteStream))
                    {
                        ...
                        if (parseCapabilitiesXml == true)
                        {
                            XDocument xmlDoc = XDocument.Parse(strResponse);

                            foreach (XNode node in xmlDoc.Nodes())
                            {
                                XElement el = node as XElement;
                                if (el != null && el.Name.LocalName.Equals("Capabilities"))
                                {
                                  foreach (XNode capChildNode in el.Nodes())
                                    {  
                                XElement capChildEl = capChildNode as XElement;
                                if (capChildEl != null && capChildEl.Name.LocalName.Equals("Contents"))
                                {
                                    foreach (XNode contentChildNode in capChildEl.Nodes())
                                    {
                                        XElement contentChildEl = contentChildNode as XElement;
                                        if (contentChildEl != null && contentChildEl.Name.LocalName.Equals("Layer"))
                                        {
                                            foreach (XNode layerChildNode in contentChildEl.Nodes())
                                            {
                                                XElement layerChildEl = layerChildNode as XElement;
                                                if (layerChildEl != null && layerChildEl.Name.LocalName.Equals("Dimension"))
                                                {
                                                    bool firstTimeElementPassed = false;
                                                    List<XNode> toDelete = new List<XNode>();
                                                    foreach (XNode dimChildNode in layerChildEl.Nodes())
                                                    {
                                                        XElement dimChildEl = dimChildNode as XElement;
                                                        if (dimChildEl != null && dimChildEl.Name.LocalName.Equals("Value"))
                                                        {
                                                            if (firstTimeElementPassed == true)
                                                            {
                                                                toDelete.Add(dimChildNode);
                                                            }
                                                            firstTimeElementPassed = true;
                                                        }
                                                    }
                                                    foreach (XNode delNode in toDelete)
                                                    {
                                                        delNode.Remove();
                                                    }
                                                }
